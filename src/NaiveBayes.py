...
# change paths
#trainJSON = add path to dataset
#testJSON = add path to dataset
...
def getData(inJSON):
    ...
    for conversation in (data['conversations']):
        ...
        TrainTexts.append(fullText)
        TrainLabels.append(conversation["isCybergrooming"])
    return TrainTexts,TrainLabels

TrainTexts,TrainLabels = getData(trainJSON)
estTexts,TestLabels = getData(testJSON)
...
train_x = vectorizer.fit_transform(TrainTexts)
train_y = label_encoder.fit_transform(TrainLabels)
